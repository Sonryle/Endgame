...
export class Inventory {
    constructor(grid) {
        ...
        this.image.attr('href', "./src/assets/textures/gui/container/numbered_inventory.png");
        ...
        this.slots[1] = new ItemSlot(xOffset, yOffset);
        this.slots[2] = new ItemSlot(xOffset + this.cellScale, yOffset);
        this.slots[3] = new ItemSlot(xOffset, yOffset + this.cellScale);
        this.slots[4] = new ItemSlot(xOffset + this.cellScale, yOffset + this.cellScale);

        // Armour
        xOffset = topLeftX + this.cellScale;
        yOffset = topLeftY + 14 * state.scale;
        this.slots[5] = new ItemSlot(xOffset, yOffset + 0 * this.cellScale);
        this.slots[6] = new ItemSlot(xOffset, yOffset + 1 * this.cellScale);
        this.slots[7] = new ItemSlot(xOffset, yOffset + 2 * this.cellScale);
        this.slots[8] = new ItemSlot(xOffset, yOffset + 3 * this.cellScale);
        ...
        for (let y = 0; y < 3; y++) {
            for (let x = 0; x < 9; x++) {
                this.slots[9 + x * (y + 1)] = new ItemSlot(xOffset + x * this.cellScale, yOffset + y * this.cellScale);
                console.log(9 + x + (y * 9));
            }
        }
        ...
        for (let x = 0; x < 9; x++) {
            this.slots[36 + x] = new ItemSlot(xOffset + x * this.cellScale, yOffset);
            console.log(36 + x);
        }

        // Offhand Slot
        this.slots[45] = new ItemSlot(topLeftX + 87 * state.scale, topLeftY + 68 * state.scale);
    }
}
