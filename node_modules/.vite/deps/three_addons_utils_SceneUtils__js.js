import { J as BufferGeometry, Rn as Group, q as BufferAttribute, ri as Mesh, st as Color, ti as Matrix4, zs as Vector3 } from "./three.module-BLr34Qqw.js";
import { n as mergeGroups, t as deepCloneAttribute } from "./BufferGeometryUtils-SbPuB3Ir.js";
//#region node_modules/three/examples/jsm/utils/SceneUtils.js
/**
* @module SceneUtils
...
function createMeshesFromMultiMaterialMesh(mesh) {
	...
	for (let i = 0; i < groups.length; i++) {
		...
		for (let j = 0; j < attributeNames.length; j++) {
			...
			const newAttribute = new BufferAttribute(new type(newLength), itemSize);
...
function createMultiMaterialObject(geometry, materials) {
	const group = new Group();
	for (let i = 0, l = materials.length; i < l; i++) group.add(new Mesh(geometry, materials[i]));
	return group;
}
/**
...
function reduceVertices(object, func, initialValue) {
	...
	object.traverseVisible((child) => {
		...
		if (geometry !== void 0) {
			...
			if (position !== void 0) for (let i = 0, l = position.count; i < l; i++) {
				if (child.isMesh) child.getVertexPosition(i, vertex);
				else vertex.fromBufferAttribute(position, i);
				if (!child.isSkinnedMesh) vertex.applyMatrix4(child.matrixWorld);
...
function sortInstancedMesh(mesh, compareFn) {
	...
	for (const name in mesh.geometry.attributes) {
		...
		if (attribute.isInstancedBufferAttribute) attributeRefs.set(attribute, deepCloneAttribute(attribute));
	}
	const tokens = [];
...
function* traverseGenerator(object) {
	...
	for (let i = 0, l = children.length; i < l; i++) yield* traverseGenerator(children[i]);
}
/**
...
function* traverseVisibleGenerator(object) {
	...
	yield object;
	const children = object.children;
	for (let i = 0, l = children.length; i < l; i++) yield* traverseVisibleGenerator(children[i]);
}
/**
...
//#endregion
export { createMeshesFromInstancedMesh, createMeshesFromMultiMaterialMesh, createMultiMaterialObject, reduceVertices, sortInstancedMesh, traverseAncestorsGenerator, traverseGenerator, traverseVisibleGenerator };
